$(document).ready(function() {
  ...
  context_data = JSON.parse(
    document.getElementById("context_json").textContent
  );
  ...
  class MotionSender {
    constructor(socket) {
      ...
      this.start = function(fps) {
        ...
        motion_sender_intervalometer_id = setInterval(
          this.send_latest,
          1000 / fps
        );
      };
  ...
  guest_socket.onmessage = function(event) {
    ...
    if (
      !received_data.feature.presenter_channel ||
      0 === received_data.feature.presenter_channel
    ) {
      var msg = "Feature unavailable.";
      $("#message_display").show();
      $("#message_display").text(msg);
    } else if (
      received_data.guest_queue[0].session_key === context_data.session_key
    ) {
